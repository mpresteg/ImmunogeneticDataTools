...
* **[`hla-report-extraction`](hla-report-extraction/README.md)** — early-stage: extracting HLA typing results (as GL Strings) from lab report PDFs, the step upstream of everything `ld-validation` already does. So far just PDF text-layer extraction, tested against one real (de-identified, lab-published) sample report; no candidate-line detection or GL String construction yet. See its README for scope and current status.
